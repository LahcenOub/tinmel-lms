...
const ProfsManagementView: React.FC<{ user: User }> = ({ user }) => {
    const { t } = useLanguage();
            ...
            <div className="lg:col-span-2">
                <div className="bg-white rounded-lg shadow p-6 mb-8">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <input className="flex-1 border rounded p-2 text-sm" placeholder={t('profName')} value={newProfName} onChange={(e) => setNewProfName(e.target.value)} />
                        <input className="flex-1 border rounded p-2 text-sm" placeholder={t('profSchool')} value={newProfSchool} onChange={(e) => setNewProfSchool(e.target.value)} />
                        <input className="flex-1 border rounded p-2 text-sm" placeholder={t('profCity')} value={newProfCity} onChange={(e) => setNewProfCity(e.target.value)} />
                        <button onClick={handleCreateProf} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-sm whitespace-nowrap">{t('genAccess')}</button>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4 items-center">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        <select className="border rounded p-2 text-sm flex-1 md:w-40" value={selectedSchoolFilter} onChange={e => { setSelectedSchoolFilter(e.target.value); setCurrentPage(1); }}>
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden mb-4">
                    <table className="w-full text-start text-sm">
                        ...
                        <tbody>
                            {paginatedProfs.map(p => (
                                <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                                    ...
                                    <td className="p-3 text-end flex justify-end gap-2">
                                        <button 
                                            onClick={() => navigate(`/moderator/communication?profId=${p.id}`)}
                            ...
                            {paginatedProfs.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-gray-500">{t('noProfs')}</td></tr>}
                        </tbody>
                    </table>
                </div>
                ...
                {totalPages > 1 && (
                    <div className="flex justify-between items-center text-sm">
                        ...
                        <div className="flex gap-2">
                            <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-1 rounded border hover:bg-gray-50 disabled:opacity-50"><ChevronLeft className="w-5 h-5 rtl:flip"/></button>
                            <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-1 rounded border hover:bg-gray-50 disabled:opacity-50"><ChevronRight className="w-5 h-5 rtl:flip"/></button>
                        </div>
                    </div>
                )}
            </div>
...
const CommunicationView: React.FC<{ user: User }> = ({ user }) => {
    const { t } = useLanguage();
    ...
    useEffect(() => {
        ...
        // Auto-select prof if ID passed in URL
    ...
    const handleBroadcastAnnouncement = () => {
        ...
        const ann: Announcement = { id: `ann-${Date.now()}`, title: announcementTitle, content: announcementContent, date: new Date().toISOString(), author: user.name };
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        const msg: Message = { id: `msg-${Date.now()}`, senderId: user.id, senderName: user.name, receiverId: selectedProfId, content: newMessage, timestamp: new Date().toISOString(), read: false };
                ...
                <div className="w-1/3 border-e bg-gray-50 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        {conversations.map(convId => {
                            ...
                            if (!prof) return null;
                            return (
                                <button key={convId} onClick={() => setSelectedProfId(convId)} className={`w-full p-4 text-start hover:bg-blue-50 transition border-b ${selectedProfId === convId ? 'bg-blue-100' : ''}`}>
    ...
    <div className="min-h-screen bg-gray-100 p-8" dir={dir}>
       <div className="max-w-6xl mx-auto">
           ...
           <div className="flex gap-4 mb-6 border-b">
               <Link 
                  to="/moderator/profs"
                  className={`pb-2 px-4 flex items-center gap-2 border-b-2 transition-colors ${isActive('profs') ? 'border-blue-600 text-blue-600 font-medium' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
               >
           ...
           <Routes>
               <Route path="profs" element={<ProfsManagementView user={user} />} />
               <Route path="communication" element={<CommunicationView user={user} />} />
               <Route path="*" element={<Navigate to="profs" replace />} />
           </Routes>
       </div>
    </div>
  );
